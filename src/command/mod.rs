...
impl<'a> Input<'a> {
    pub fn new(arg: Option<&'a str>, from_stdin: bool) -> Self {
        Input { arg, from_stdin }
    }

    fn value(&self) -> AppResult<Option<Cow<'a, str>>> {
       if self.from_stdin {
           let stdin = io::stdin();
...
mod arg {
    ...
    pub trait Value: Command {
        fn value(&self) -> Input {
            Input::new(self.matches().value_of("value"), self.matches().is_present("stdin"))
        }
    }
